...
export default function HomePage() {
  ...
  return (
    <div className="flex-1 w-full pb-32 md:pb-10 px-5 pt-4">
      {/* Header */}
      <div className="flex flex-row items-center justify-between mb-5">
        <button className="md:hidden text-left active:opacity-70 transition-opacity" onClick={() => router.push("/cigarette-tracker")}>
          <BrandLockup compact />
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-x-8 gap-y-6">
        ...
        <div className="md:col-span-5 flex flex-col gap-0">
          ...
          <div className="fixed bottom-8 right-5 z-40 md:bottom-auto md:right-auto md:relative md:mt-4 md:flex md:justify-center md:w-full">
            <button
              onClick={() => router.push('/transaction?kind=expense')}
              className="w-14 h-14 md:w-full md:max-w-[300px] md:h-[52px] rounded-full md:rounded-2xl flex flex-row items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform"
              style={{ backgroundColor: palette.primary, boxShadow: `0 10px 15px -3px ${palette.primary}40` }}
            >
              <span className="text-white text-[24px] md:text-[14px] font-extrabold leading-none">+</span>
              <span className="hidden md:inline text-white text-[14px] font-extrabold">{t("addExpense")}</span>
            </button>
          </div>
