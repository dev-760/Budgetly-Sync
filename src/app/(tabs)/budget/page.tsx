...
export default function BudgetScreen() {
  ...
  return (
    <div className="flex flex-col h-full w-full px-5 overflow-y-auto" style={{ backgroundColor: palette.background }}>
      <div className="pt-3.5 pb-32 md:pb-11">
        {/* Header */}
        <div className="flex flex-row items-center justify-end mb-[18px]">
          <button
            onClick={() => router.push('/budget-edit')}
            className="h-[42px] px-4 rounded-xl flex items-center justify-center gap-2 active:opacity-70 transition-opacity"
            style={{ backgroundColor: palette.primary, boxShadow: `0 4px 12px ${palette.primary}40` }}
          >
            <Plus size={18} color="white" />
            <span className="text-white text-[13px] font-bold">{label("Add Budget", "Ajouter un budget")}</span>
          </button>
        </div>
